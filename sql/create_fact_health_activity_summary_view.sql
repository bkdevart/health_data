CREATE OR REPLACE VIEW fact_health_activity_summary AS
SELECT 
...
LEFT JOIN 
    (SELECT customer_id, start_date, source_name,
     ...
     GROUP BY customer_id, start_date, source_name) cycling
ON heart_rate.customer_id = cycling.customer_id AND heart_rate.start_date = cycling.start_date
  AND heart_rate.source_name = cycling.source_name
LEFT JOIN 
    (SELECT customer_id, start_date, source_name,
...
ON heart_rate.customer_id = walk_run.customer_id AND heart_rate.start_date = walk_run.start_date
  AND heart_rate.source_name = walk_run.source_name
LEFT JOIN 
    (SELECT customer_id, start_date, source_name,
     ...
     GROUP BY customer_id, start_date, source_name) steps
...
LEFT JOIN 
    (SELECT customer_id, start_date, source_name,
     ...
     GROUP BY customer_id, start_date, source_name) basal
